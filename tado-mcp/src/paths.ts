...
const DEFAULT_APP_SUPPORT =
  process.env.TADO_STORAGE_DEFAULT_ROOT ||
...
export function appSupportRoot(): string {
  ...
  if (!existsSync(locator)) return DEFAULT_APP_SUPPORT;
  try {
    const parsed = JSON.parse(readFileSync(locator, "utf-8")) as {
      activeRoot?: string;
    };
...
export function findProjectRoot(cwd: string = process.cwd()): string | null {
  let dir = cwd;
  while (dir !== sep) {
    if (existsSync(join(dir, ".tado"))) return dir;
...
export function projectLocalPath(cwd?: string): string | null {
  const root = findProjectRoot(cwd);
  return root ? join(root, ".tado", "local.json") : null;
}

export function projectMemoryPath(cwd?: string): string | null {
  const root = findProjectRoot(cwd);
  return root ? join(root, ".tado", "memory", "project.md") : null;
}

export function projectNotesDir(cwd?: string): string | null {
  const root = findProjectRoot(cwd);
...
export function writeJSONSyncAtomic(path: string, value: unknown): void {
  const parent = path.split(sep).slice(0, -1).join(sep);
  if (parent) mkdirSync(parent, { recursive: true });
  const tmp = `${path}.tmp-${process.pid}`;
