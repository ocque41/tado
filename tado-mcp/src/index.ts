...
server.registerTool("tado_list", {
  ...
  description:
    "List all active Tado terminal sessions with their ID, engine, grid position, status, project, and name. Use this to discover which AI coding agents are currently running in Tado before sending messages or reading their output.",
  inputSchema: {
...
server.registerTool("tado_read", {
  ...
  description:
    "Read the terminal output of a Tado session. Target can be a grid coordinate (e.g. '1,1'), session name substring, or UUID. Use tado_list first to see available targets.",
  inputSchema: {
    target: z
      .string()
...
server.registerTool("tado_send", {
  ...
  description:
    "Send typed input to a Tado terminal session. The message will appear as if typed into the terminal. Target can be a grid coordinate (e.g. '1,1'), session name substring, or UUID. Use tado_list first to see available targets.",
  inputSchema: {
    target: z
      .string()
      .describe("Grid coord (1,1), name substring, or UUID"),
...
server.registerTool("tado_broadcast", {
  ...
  description:
    "Send a message to all active Tado sessions, optionally filtered by project or team. Useful for coordinating multiple agents simultaneously.",
  inputSchema: {
...
server.registerTool("tado_config_get", {
  ...
  description:
    "Read a dotted key from Tado's settings. Scope: 'global' (user-wide), 'project' (committed .tado/config.json), or 'project-local' (gitignored .tado/local.json). Project scope requires running inside a Tado project.",
  inputSchema: {
...
server.registerTool("tado_config_set", {
  ...
  description:
    "Atomically update a dotted key in Tado's settings. Write is serialized through the same rename-based atomic store used by the Swift app, so edits from multiple sources don't tear.",
  inputSchema: {
    scope: z.string().describe("global | project | project-local"),
...
server.registerTool("tado_memory_read", {
  ...
  description:
    "Return the markdown content of Tado's long-lived memory. Scope: 'user' (cross-project) or 'project' (this project only, requires running inside a Tado project).",
  inputSchema: {
...
server.registerTool("tado_memory_append", {
  ...
  description:
    "Append a timestamped note to Tado memory. Use this to persist durable facts across runs — user preferences, decisions, gotchas. Notes survive app restart and show up in future agent spawns (for project scope) or across every agent (for user scope).",
  inputSchema: {
...
server.registerTool("tado_memory_search", {
  ...
  description:
    "Case-insensitive substring search across user + project memory. Returns up to 100 hits with file:line:context.",
  inputSchema: {
...
server.registerTool("tado_notify", {
  ...
  description:
    "Emit a user-visible event to Tado's notification system — shows an in-app banner and (if the app is backgrounded) a macOS system notification. Useful for agents to surface milestones or failures without shouting in the terminal.",
  inputSchema: {
...
server.registerTool("tado_events_query", {
  ...
  description:
    "Read the most recent Tado events, filtered by type/severity/since. Returns newest-first, up to the specified limit.",
  inputSchema: {
