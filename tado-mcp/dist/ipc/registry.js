...
export function resolveTarget(entries, target, projectFilter, teamFilter) {
    let filtered = entries;
    if (projectFilter) {
        filtered = filtered.filter((e) => e.projectName?.toLowerCase() === projectFilter.toLowerCase());
    }
    if (teamFilter) {
        filtered = filtered.filter((e) => e.teamName?.toLowerCase() === teamFilter.toLowerCase());
    }
    // 1. Exact UUID match
