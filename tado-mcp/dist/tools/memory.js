        ...
        case "project":
            {
                ...
                if (!p)
                    throw new Error(`No .tado/ directory found above ${process.cwd()} — cannot resolve project memory scope.`);
                return p;
            }
        default:
...
export async function tadoMemoryRead(args) {
    ...
    if (!existsSync(path))
        return `(empty — no ${scope} memory file yet at ${path})`;
    return await readFile(path, "utf-8");
}
export async function tadoMemoryAppend(args) {
    if (!args.text.trim())
        return "refusing to append empty note";
    const scope = args.scope ?? "project";
    const path = resolveFile(scope);
    await ensureFile(path);
    const iso = new Date().toISOString();
    const tagLine = args.tags && args.tags.length > 0 ? `\n_tags: ${args.tags.join(", ")}_` : "";
    const entry = `\n\n## ${iso}${tagLine}\n\n${args.text.trim()}\n`;
...
export async function tadoMemorySearch(args) {
    ...
    if (scope === "user" || scope === "all")
        files.push(paths.userMemoryMarkdown);
        ...
        if (notesDir && existsSync(notesDir)) {
            for (const entry of readdirSync(notesDir)) {
                if (entry.endsWith(".md"))
                    files.push(join(notesDir, entry));
            }
        }
    }
    const hits = [];
