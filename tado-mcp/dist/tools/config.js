import { paths, projectConfigPath, projectLocalPath, readJSON, writeJSONAtomic, } from "../paths.js";
...
function resolvePath(scope) {
    switch (scope) {
        ...
        case "project-shared":
            {
                ...
                if (!p)
                    throw new Error(`No .tado/ directory found above ${process.cwd()}`);
                return p;
            }
        case "project-local":
        case "local":
        ...
        default:
            throw new Error(`unknown scope: ${scope} (expected: global | project | project-local)`);
    }
}
function navigate(obj, key) {
    let cursor = obj;
