...
| 2.18 | Glyph atlas overflow recovery: on shelf exhaustion, reset + retry; default size bumped 2048² → 4096² (~4× capacity). modCount bump already triggers GPU lookup rebuild. | ✅ shipped |
| 2.19 | Astral-plane codepoint rendering: atlas rasterizes > U+FFFF scalars via surrogate-pair + CoreText font fallback; renderer remaps to dense slots 0x10000..0x11000 (BMP-above slots) so the GPU lookup covers them. Emoji renders monochrome for now (R8 atlas — color RGBA atlas is 2.20). | ✅ shipped (48/48 tests green) |
