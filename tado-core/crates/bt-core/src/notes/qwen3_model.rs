// Vendored from `candle-transformers-0.10.2/src/models/qwen3.rs`.
// Two changes vs upstream:
//  1. Imports rewritten to use `candle_nn` directly (the upstream
//     `with_tracing` wrappers are tracing-instrumented; we don't need
//     the spans and we don't want a dep on the rest of
//     candle-transformers).
//  2. `Model::clear_kv_cache` is `pub` (was crate-private). Embedding
//     models call `forward(seq, 0)` once per input and must reset the
