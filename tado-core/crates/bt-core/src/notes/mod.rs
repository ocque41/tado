...
//!                  migration_18. No vector column yet — that lands when
//!                  the embedder ships.
//! - [`embeddings`] — public trait + metadata-aware providers. Qwen3 is
//!                  the default provider shape; it currently keeps a
//!                  deterministic local fallback until the bundled model
//!                  runtime lands.
//! - [`search`]   — hybrid search API. For now the vector path is a
//!                  no-op and hits fall back to the existing FTS5
//!                  search at doc granularity. The API shape (`SearchHit`,
//!                  `hybrid_search`) is final; swapping in real
//!                  embeddings only changes the body of
...
pub use search::{hybrid_search, HybridQuery, SearchHit};
