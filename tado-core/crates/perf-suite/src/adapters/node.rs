...
impl Adapter for NodeAdapter {
    ...
    fn measure(
        ...
        samples.insert(
            ...
            alloc_per_op::sample(allocs, "node", allocs_note.clone()),
        );
        ...
        samples.insert(
            ...
            critical_path_ops::sample(cp, "node", cp_note.clone()),
        );
        ...
        samples.insert(
            ...
            io_syscalls_per_op::sample(sys, "node", sys_note.clone()),
        );
        ...
        samples.insert(
            ...
            db_query_cost::sample(db, "node", db_note.clone()),
        );
        ...
        samples.insert(
            ...
            xproc_roundtrips::sample(xp, "node", xp_note.clone()),
        );
        ...
        samples.insert(
            ...
            cold_start_ops::sample(cold, "node", cold_note.clone()),
        );
        ...
        samples.insert(
            ...
            steady_state_rss_ratio::sample(rss, "node", rss_note.clone()),
        );
...
fn measure_node_alloc(root: &Path, run_dir: &Path) -> (f64, Option<String>) {
    ...
    let entries = match std::fs::read_dir(&snapshot_dir) {
        ...
        Err(_) => {
            return (
                0.0,
                Some("alloc_per_op: V8 produced no heap profile".into()),
            )
        }
    };
    ...
    if node_count == 0.0 {
        return (
            0.0,
            Some("alloc_per_op: heap profile contained no samples".into()),
        );
    }
    (
        node_count,
...
fn measure_node_critical_path(root: &Path, run_dir: &Path) -> (f64, Option<String>) {
    let entry = match detect_node_entry(root) {
        ...
        None => {
            return (
                0.0,
                Some("critical_path_ops: no entry script detected".into()),
            )
        }
    };
    ...
    let entries = match std::fs::read_dir(&prof_dir) {
        ...
        Err(_) => {
            return (
                0.0,
                Some("critical_path_ops: V8 produced no CPU profile".into()),
            )
        }
    };
    ...
    if sample_count == 0.0 {
        return (
            0.0,
            Some("critical_path_ops: CPU profile contained no samples".into()),
        );
    }
    (
        sample_count,
        Some(format!(
            "critical_path_ops: V8 CPU profile captured {sample_count:.0} samples"
        )),
    )
}
...
fn measure_node_algo(root: &Path) -> (f64, Option<String>) {
    ...
    match worst {
        Some((group, slope)) => (
            slope,
            Some(format!(
                "algo_complexity: worst slope {slope:.3} from group '{group}'"
            )),
        ),
        None => (
            1.0,
