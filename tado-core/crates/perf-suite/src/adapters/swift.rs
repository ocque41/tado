...
impl Adapter for SwiftAdapter {
    ...
    fn measure(
        ...
        samples.insert(
            ...
            alloc_per_op::sample(allocs, "swift", allocs_note.clone()),
        );
        ...
        samples.insert(
            ...
            critical_path_ops::sample(cp, "swift", cp_note.clone()),
        );
        ...
        samples.insert(
            ...
            io_syscalls_per_op::sample(sys, "swift", sys_note.clone()),
        );
        ...
        samples.insert(
            ...
            db_query_cost::sample(db, "swift", db_note.clone()),
        );
        ...
        samples.insert(
            ...
            xproc_roundtrips::sample(ffi, "swift", ffi_note.clone()),
        );
        ...
        samples.insert(
            ...
            cold_start_ops::sample(cold, "swift", cold_note.clone()),
        );
        ...
        samples.insert(
            ...
            steady_state_rss_ratio::sample(rss, "swift", rss_note.clone()),
        );
...
fn measure_swift_alloc(root: &Path) -> (f64, Option<String>) {
    if std::env::var("TADO_PERF_INSTRUMENTS").as_deref() != Ok("1") {
        return (
            0.0,
            Some("alloc_per_op: Instruments mode opt-in (set TADO_PERF_INSTRUMENTS=1)".into()),
        );
    }
    if which("xctrace").is_none() {
        return (
            0.0,
            Some("alloc_per_op: xctrace not installed (Xcode required)".into()),
        );
    }
        ...
        Ok(out) => {
            ...
            if let Some(cap) = re.captures(&combined) {
                if let Ok(n) = cap.get(1).unwrap().as_str().parse::<f64>() {
                    return (
                        n,
                        Some(format!("alloc_per_op: xctrace counted {n:.0} allocations")),
                    );
                }
            }
            (
                0.0,
                Some("alloc_per_op: xctrace produced no Total Allocations line".into()),
            )
        }
...
fn measure_swift_critical_path(root: &Path) -> (f64, Option<String>) {
    if std::env::var("TADO_PERF_INSTRUMENTS").as_deref() != Ok("1") {
        return (
            0.0,
            Some("critical_path_ops: Instruments mode opt-in".into()),
        );
    }
    if which("xctrace").is_none() {
    ...
    match output {
        Ok(out) => {
            ...
            if let Some(cap) = re.captures(&combined) {
                if let Ok(n) = cap.get(1).unwrap().as_str().parse::<f64>() {
                    return (
                        n,
                        Some(format!(
                            "critical_path_ops: Time Profiler captured {n:.0} samples"
                        )),
                    );
                }
            }
            (
                0.0,
                Some("critical_path_ops: xctrace produced no sample count".into()),
            )
        }
        Err(e) => (
            0.0,
            Some(format!("critical_path_ops: xctrace failed ({e})")),
        ),
    }
}
...
fn measure_swift_io_syscalls(root: &Path) -> (f64, Option<String>) {
    let target = match swift_executable_target(root) {
        ...
        None => {
            return (
                0.0,
                Some("io_syscalls_per_op: no swift executable target detected".into()),
            )
        }
    };
...
fn measure_swift_cold_start(root: &Path) -> (f64, Option<String>) {
    let target = match swift_executable_target(root) {
        ...
        None => {
            return (
                0.0,
                Some("cold_start_ops: no swift executable target detected".into()),
            )
        }
    };
...
fn measure_swift_rss(root: &Path) -> (f64, Option<String>) {
    let target = match swift_executable_target(root) {
        ...
        None => {
            return (
                1.0,
                Some("steady_state_rss_ratio: no swift executable target detected".into()),
            )
        }
    };
...
fn swift_executable_target(root: &Path) -> Option<SpawnTarget> {
    ...
    let bin_dir = String::from_utf8_lossy(&bin_path_out.stdout)
        .trim()
        .to_string();
    if bin_dir.is_empty() {
        return None;
...
fn measure_swift_algo_complexity(root: &Path) -> (f64, Option<String>) {
    if !root.join("Package.swift").exists() {
        return (
            1.0,
            Some(
                "algo_complexity: Package.swift not found (Xcode-only projects unsupported)".into(),
            ),
        );
    }
    let output = Command::new("swift")
    ...
    let Ok(output) = output else {
        return (
            1.0,
            Some("algo_complexity: swift test failed to spawn".into()),
        );
    };
    let combined = format!(
        "{}\n{}",
    ...
    let re = Regex::new(r#"testCase\s+'-\[\w+\s+(\w+_n(\d+))\]'\s+passed.*\(([\d.]+)\s+seconds\)"#)
        .unwrap();
...
fn measure_swift_xproc(root: &Path) -> (f64, Option<String>) {
    ...
    if count == 0 {
        return (
            0.0,
            Some("xproc_roundtrips: no FFI / @objc bridges detected".into()),
        );
    }
    (
        count as f64,
        Some(format!(
            "xproc_roundtrips: {count} bridge declarations / decorators"
        )),
    )
}
...
fn is_skip_dir(name: &str) -> bool {
    matches!(
        name,
        ".build" | "DerivedData" | "node_modules" | ".git" | ".tado" | "Pods"
    )
}
...
pub(super) fn default_samples(adapter: &str) -> BTreeMap<String, MetricSample> {
    ...
    samples.insert(
        algo_complexity::NAME.to_string(),
        algo_complexity::sample_from_slope(1.0, adapter, None),
    );
    samples.insert(
        alloc_per_op::NAME.to_string(),
        alloc_per_op::sample(0.0, adapter, None),
    );
    samples.insert(
        critical_path_ops::NAME.to_string(),
        critical_path_ops::sample(0.0, adapter, None),
    );
    samples.insert(
        io_syscalls_per_op::NAME.to_string(),
        io_syscalls_per_op::sample(0.0, adapter, None),
    );
    samples.insert(
        db_query_cost::NAME.to_string(),
        db_query_cost::sample(0.0, adapter, None),
    );
    samples.insert(
        xproc_roundtrips::NAME.to_string(),
        xproc_roundtrips::sample(0.0, adapter, None),
    );
    samples.insert(
        cold_start_ops::NAME.to_string(),
        cold_start_ops::sample(0.0, adapter, None),
    );
    samples.insert(
        steady_state_rss_ratio::NAME.to_string(),
        steady_state_rss_ratio::sample(1.0, adapter, None),
    );
    samples
}
