...
impl Adapter for PythonAdapter {
    ...
    fn measure(
        ...
        samples.insert(
            ...
            alloc_per_op::sample(allocs, "python", allocs_note.clone()),
        );
        ...
        samples.insert(
            ...
            critical_path_ops::sample(cp, "python", cp_note.clone()),
        );
        ...
        samples.insert(
            ...
            io_syscalls_per_op::sample(sys, "python", sys_note.clone()),
        );
        ...
        samples.insert(
            ...
            db_query_cost::sample(db, "python", db_note.clone()),
        );
        ...
        samples.insert(
            ...
            xproc_roundtrips::sample(xp, "python", xp_note.clone()),
        );
        ...
        samples.insert(
            ...
            cold_start_ops::sample(cold, "python", cold_note.clone()),
        );
        ...
        samples.insert(
            ...
            steady_state_rss_ratio::sample(rss, "python", rss_note.clone()),
        );
    ...
    let entry = match detect_python_entry(root) {
        ...
        None => {
            return (
                0.0,
                Some(
                    "alloc_per_op: no entry script (looked for main.py / app.py / __main__.py)"
                        .into(),
                ),
            )
        }
...
"#,
    ...
    if std::fs::write(&shim, shim_src).is_err() {
        return (
            0.0,
            Some("alloc_per_op: failed to write tracemalloc shim".into()),
        );
    }
    ...
    let output = match run_with_budget(cmd, Some(Duration::from_secs(20))) {
        ...
        Err(e) => {
            return (
                0.0,
                Some(format!("alloc_per_op: tracemalloc shim failed ({e})")),
            )
        }
    };
    let combined = format!(
    ...
    if let Some(cap) = re.captures(&combined) {
        if let Ok(n) = cap.get(1).unwrap().as_str().parse::<f64>() {
            return (
                n,
                Some(format!("alloc_per_op: tracemalloc peak {n:.0} bytes")),
            );
        }
    }
    (
        0.0,
        Some("alloc_per_op: tracemalloc shim produced no peak line".into()),
    )
}
...
"#,
    ...
    if std::fs::write(&shim, shim_src).is_err() {
        return (
            0.0,
            Some("critical_path_ops: failed to write cProfile shim".into()),
        );
    }
    let mut cmd = Command::new("python3");
    cmd.arg(&shim).current_dir(root);
    let output = match run_with_budget(cmd, Some(Duration::from_secs(20))) {
        ...
        Err(e) => {
            return (
                0.0,
                Some(format!("critical_path_ops: cProfile shim failed ({e})")),
            )
        }
    };
    let combined = format!(
    ...
    if let Some(cap) = re.captures(&combined) {
        if let Ok(n) = cap.get(1).unwrap().as_str().parse::<f64>() {
            return (
                n,
                Some(format!(
                    "critical_path_ops: cProfile counted {n:.0} function calls"
                )),
            );
        }
    }
    (
        0.0,
        Some("critical_path_ops: cProfile shim produced no count line".into()),
    )
}
...
fn measure_python_algo(root: &Path, run_dir: &Path) -> (f64, Option<String>) {
    ...
    let Ok(_) = output else {
        return (
            1.0,
            Some("algo_complexity: pytest-benchmark not installed".into()),
        );
    };
    let Ok(text) = std::fs::read_to_string(&json_out) else {
        return (
            1.0,
            Some("algo_complexity: no pytest-benchmark output produced".into()),
        );
    };
    ...
    let value: serde_json::Value = match serde_json::from_str(&text) {
        ...
        Err(_) => {
            return (
                1.0,
                Some("algo_complexity: pytest-benchmark JSON parse failed".into()),
            )
        }
    };
    ...
    if let Some(arr) = value.get("benchmarks").and_then(|v| v.as_array()) {
        for b in arr {
            ...
            let Some(stats) = b.get("stats").and_then(|v| v.as_object()) else {
                continue;
            };
            let Some(mean) = stats.get("mean").and_then(|v| v.as_f64()) else {
                continue;
            };
