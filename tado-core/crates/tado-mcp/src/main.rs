...
fn tool_definitions() -> Value {
    json!([
        ...
        {
            "name": "tado_pets_hatch",
            "description": "Hatch a Tado Pets sprite from a free-text prompt. v1 stub: writes a placeholder sprite into <storage-root>/pets/custom/<uuid>.png and returns the file path. Real image generation lights up later — the file shape, naming, and return contract are stable across the upgrade.",
            "inputSchema": {
                "type": "object",
                "required": ["prompt"],
        ...
        {
            "name": "tado_pets_list_states",
            "description": "Return the canonical list of Tado Pets states the floating companion can play. Use before assigning images to states so you reference stable raw values.",
            "inputSchema": { "type": "object", "properties": {} }
        },
        {
            "name": "tado_pets_import_slice",
            "description": "Slice a sprite-sheet image into rows*cols cells, writing each cell as cell-<row>-<col>.png into outputDir. Returns the list of written paths plus the source dimensions. Used by the tado-pets-importer agent when it detects a single-image sprite sheet.",
            "inputSchema": {
                "type": "object",
                "required": ["sourceImage", "outputDir"],
                "properties": {
                    "sourceImage": { "type": "string", "description": "Absolute path to the source image." },
                    "rows": { "type": "integer", "minimum": 1, "default": 2 },
                    "cols": { "type": "integer", "minimum": 1, "default": 4 },
                    "outputDir": { "type": "string", "description": "Absolute output dir; created if absent." }
                }
            }
        },
        {
            "name": "tado_pets_register",
            "description": "Register a custom Tado Pet by writing meta.json to <storage-root>/pets/custom/<id>/. Each state in the states map points at a relative path inside that directory (file or folder). Returns the registered id and the absolute pet directory.",
            "inputSchema": {
                "type": "object",
                "required": ["id", "name", "states"],
                "properties": {
                    "id": { "type": "string", "description": "Stable id without the 'custom:' prefix." },
                    "name": { "type": "string" },
                    "states": {
                        "type": "object",
                        "description": "Map from PetState raw value to relative path under the pet dir.",
                        "additionalProperties": { "type": "string" }
                    },
                    "frameDurationSeconds": { "type": "number", "minimum": 0.01, "default": 0.12 },
...
async fn call_tool(name: &str, args: Value) -> Result<String> {
    match name {
        ...
        "tado_pets_hatch" => tado_pets_hatch(args),
        "tado_pets_list_states" => tado_pets_list_states(args),
        "tado_pets_import_slice" => tado_pets_import_slice(args),
        "tado_pets_register" => tado_pets_register(args),
...
/// Path to the custom-pets directory under the active storage
/// root. Mirrors `PetsHatchService.customPetsDirectory` in the
/// Swift app so the two surfaces agree on a single location.
fn pets_custom_dir() -> PathBuf {
    app_support_root().join("pets").join("custom")
}

/// `tado_pets_hatch` — v1 stub. Writes a 1×1 transparent PNG
/// placeholder under `<storage-root>/pets/custom/<uuid>.png` so
/// MCP clients can validate the round-trip. The Swift app's
/// `PetsHatchService` is the production path used when the
/// hatch is initiated from inside Tado; this MCP entry exists
/// so external agents can trigger the same flow.
fn tado_pets_hatch(args: Value) -> Result<String> {
    let prompt = args
        .get("prompt")
        .and_then(|v| v.as_str())
        .unwrap_or("")
    ...
    let dir = pets_custom_dir();
    fs::create_dir_all(&dir).map_err(|e| anyhow!("create custom pets dir: {e}"))?;
    let id = Uuid::new_v4();
    let file = dir.join(format!("{id}.png"));

    // Minimal valid PNG (1×1 transparent). Real generation
    // arrives later — the file path / naming convention is the
    // stable contract.
    const STUB_PNG: &[u8] = &[
        ...
        b'T', 0x78, 0x9C, 0x62, 0x00, 0x01, 0x00, 0x00,
        0x05, 0x00, 0x01, 0x0D, 0x0A, 0x2D, 0xB4, 0x00,
        0x00, 0x00, 0x00, b'I', b'E', b'N', b'D', 0xAE,
        0x42, 0x60, 0x82,
    ];
    fs::write(&file, STUB_PNG).map_err(|e| anyhow!("write stub sprite: {e}"))?;

    Ok(json!({
        "status": "stub",
        "path": file.to_string_lossy(),
        "prompt": prompt,
        "message": "v1 stub — wrote a placeholder PNG. Real image generation arrives later; the file path and naming convention are stable."
    })
    .to_string())
}

/// `tado_pets_list_states` — return the canonical PetState raw
/// values + display labels + priority. The list is fixed at
/// build time but ships from MCP so external agents don't have
/// to embed a copy of the enum.
fn tado_pets_list_states(_args: Value) -> Result<String> {
    let states = json!([
        { "rawValue": "idle",             "displayName": "Idle",              "priority": 10 },
        { "rawValue": "done",             "displayName": "Done",              "priority": 20 },
        { "rawValue": "needsInput",       "displayName": "Needs input",       "priority": 30 },
        { "rawValue": "running",          "displayName": "Running",           "priority": 40 },
        { "rawValue": "eternalRunning",   "displayName": "Eternal running",   "priority": 50 },
        { "rawValue": "awaitingResponse", "displayName": "Awaiting response", "priority": 60 },
        { "rawValue": "perfRegressed",    "displayName": "Perf regressed",    "priority": 70 }
    ]);
    Ok(json!({ "states": states }).to_string())
}

/// `tado_pets_import_slice` — slice a sprite-sheet image into
/// rows*cols cells, writing each cell as cell-<row>-<col>.png.
/// Used by the `tado-pets-importer` agent when it detects a
/// single-image sprite sheet.
fn tado_pets_import_slice(args: Value) -> Result<String> {
    let source = args
        .get("sourceImage")
        .and_then(Value::as_str)
        .ok_or_else(|| anyhow!("tado_pets_import_slice: missing `sourceImage`"))?;
    let output_dir_s = args
        .get("outputDir")
        .and_then(Value::as_str)
        .ok_or_else(|| anyhow!("tado_pets_import_slice: missing `outputDir`"))?;
    let rows = args
        .get("rows")
    ...
    fs::create_dir_all(&output_dir).map_err(|e| anyhow!("create outputDir: {e}"))?;

    let img = image::open(source).map_err(|e| anyhow!("open sourceImage: {e}"))?;
    let width = img.width();
    ...
    if cell_w == 0 || cell_h == 0 {
        return Err(anyhow!(
            "image too small for {}x{} grid (got {}x{})",
            rows,
            cols,
            width,
            height
    ...
    let mut written = Vec::with_capacity((rows * cols) as usize);
    for r in 0..rows {
        for c in 0..cols {
            let x = c * cell_w;
            let y = r * cell_h;
            let cell = image::imageops::crop_imm(&img, x, y, cell_w, cell_h).to_image();
            let path = output_dir.join(format!("cell-{r}-{c}.png"));
            cell.save_with_format(&path, image::ImageFormat::Png)
                .map_err(|e| anyhow!("write cell ({r},{c}): {e}"))?;
...
/// `tado_pets_register` — write `<storage-root>/pets/custom/<id>/meta.json`
/// pointing per-state at the relative paths the caller has already
/// staged inside that directory. Refuses to write outside the pet's
/// own folder; refuses to overwrite unless `overwrite: true`.
fn tado_pets_register(args: Value) -> Result<String> {
    let id = args
        .get("id")
        .and_then(Value::as_str)
        .ok_or_else(|| anyhow!("tado_pets_register: missing `id`"))?
        .trim()
        .to_string();
    if id.is_empty() {
        return Err(anyhow!("tado_pets_register: `id` is empty"));
    }
    if id.contains('/') || id.contains("..") {
        return Err(anyhow!(
            "tado_pets_register: `id` cannot contain '/' or '..'"
        ));
    }
    let name = args
        .get("name")
    ...
    let states = args
        ...
        .ok_or_else(|| anyhow!("tado_pets_register: missing `states` object"))?;
    ...
    let pet_dir = pets_custom_dir().join(&id);
    let meta_path = pet_dir.join("meta.json");

    if meta_path.exists() && !overwrite {
        return Err(anyhow!(
            "tado_pets_register: pet '{id}' already exists; pass overwrite=true to replace"
        ));
    }

    fs::create_dir_all(&pet_dir).map_err(|e| anyhow!("create pet dir: {e}"))?;

    // Validate every relative path stays inside the pet dir.
    let mut sanitised = serde_json::Map::new();
    for (key, value) in states {
        let rel = value
            .as_str()
            .ok_or_else(|| anyhow!("states.{key} must be a string"))?;
        if rel.contains("..") || rel.starts_with('/') {
            return Err(anyhow!(
                "states.{key} must be relative to the pet directory"
            ));
        }
        let abs = pet_dir.join(rel);
        if !abs.starts_with(&pet_dir) {
            return Err(anyhow!("states.{key} escapes the pet directory"));
        }
        sanitised.insert(key.clone(), Value::String(rel.to_string()));
    }
    ...
    let meta = json!({
        ...
        "states": Value::Object(sanitised),
        "createdAt": now,
        "updatedAt": now
    });
    let pretty = serde_json::to_string_pretty(&meta)
        .map_err(|e| anyhow!("serialize meta.json: {e}"))?;

    // Atomic write: temp + rename. Mirrors AtomicStore on the
    // Swift side — same guarantee that readers never see a
    // partial meta.json.
    let tmp = pet_dir.join(".meta.json.tmp");
    let mut handle = fs::File::create(&tmp).map_err(|e| anyhow!("open meta tmp: {e}"))?;
    handle
        .write_all(pretty.as_bytes())
        .map_err(|e| anyhow!("write meta tmp: {e}"))?;
    handle.sync_all().ok();
    drop(handle);
    fs::rename(&tmp, &meta_path).map_err(|e| anyhow!("rename meta tmp: {e}"))?;

    Ok(json!({
        "id": format!("custom:{id}"),
        "path": pet_dir.to_string_lossy(),
        "metaPath": meta_path.to_string_lossy(),
        "stateCount": states.len()
    })
    .to_string())
}
